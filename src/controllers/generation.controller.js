...
async function generateRandomOutfit(targetImageUrl) {
  try {
    if (!process.env.OPENROUTER_API_KEY) {
      return "a stylish, fashionable outfit that suits the scene";
    }
    ...
    try {
      ...
      if (!imgResponse.ok) throw new Error("Failed to fetch");
    ...
    const completion = await grok.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: `You are a fashion stylist. Look at this image and suggest a DIFFERENT, randomized outfit for the person to wear instead. The outfit should suit the setting/mood of the scene but be completely different from what is currently worn. Output ONLY a short outfit description (max 30 words), no explanations. Examples: "a red lace bodysuit with black thigh-high stockings", "a sheer white negligee with delicate satin straps", "a black leather corset with matching garter belt"`,
        },
        {
          role: "user",
          content: [
            { type: "text", text: "Suggest a random different outfit for this scene:" },
            imageContent,
          ],
        },
      ],
    ...
    const outfit = completion.choices[0]?.message?.content?.trim();
    if (outfit) {
      console.log(`👗 Grok suggested random outfit: ${outfit}`);
      return outfit;
    }
    return "a stylish, fashionable outfit that suits the scene";
  } catch (error) {
    console.error("❌ Failed to generate random outfit:", error.message);
    return "a stylish, fashionable outfit that suits the scene";
  }
}
...
export async function generateCompleteRecreation(req, res) {
  ...
  try {
    ...
    console.log("\n📍 STEP 1/2: Submitting image to WaveSpeed (inputs already on Blob)...");
...
export async function generatePromptBasedImage(req, res) {
  ...
  try {
    ...
    console.log(`\n${useSeedream ? "🌙" : "🍌"} PROMPT-BASED GENERATION (${useSeedream ? "WaveSpeed Seedream V4.5 Edit" : "KIE Nano Banana Pro"})`);
...
async function processPromptImageInBackground(
  ...
  try {
    ...
    const modelName = useSeedream ? "WaveSpeed Seedream V4.5 Edit" : "KIE Nano Banana Pro";
    ...
    if (result.success && result.deferred && result.taskId) {
      await prisma.generation.update({
        where: { id: generationId },
        data: {
          replicateModel: useSeedream ? `wavespeed-seedream:${result.taskId}` : `kie-task:${result.taskId}`,
        },
      });
